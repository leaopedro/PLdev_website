...
Helper.setCookie = function setCookie(cname, cvalue, exdays) {
  var d = new Date();
...
Helper.getCookie = function getCookie(cname) {
  var name = cname + "=";
...
Helper.deleteCookie = function deleteCookie(name) {
  if (getCookie(name)) {
...
Helper.saveUserLanguage = function (lang){
  this.setCookie('4all_user_preference_language', lang, 200);
}

Helper.getUserLanguage = function () {
  return this.getCookie('4all_user_preference_language')
}
...
$(document).ready(function () {
  console.log('%c Você é desenvolvedor? Saiba que estamos contratando, veja: http://4all.com/querofazerparte/', 'background: #222; color: #bada55')
  //Alteração Bruno 14/06/2016
  //a lingua de preferência do usuário fica armazenada em cookie
  userLang = Helper.getUserLanguage();
  if (!userLang)
    userLang = (navigator.language || navigator.userLanguage) || 'pt-br';
  ...
  $('.translate-page').on('click', function(){
    ...
    Helper.saveUserLanguage(userLang);
    translate(userLang);

    $(window).trigger('translatedPage');
    // window.scrollTo(0,0);
    // if($('#main-verticals-animation').length > 0){
    //   cancelAnimation();
    // }
...
},{"./Helper.js":1,"./translate.js":5}],3:[function(require,module,exports){
module.exports={
