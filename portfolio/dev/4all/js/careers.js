...
var loadJobs = require('./populate-jobs.js');
...
$(window).load(function () {
  ...
  $('.job-title').on('click', function () {
    ...
    if (active){
      // console.log('tem active', active, job[0]);
      if(active == job[0]){
        console.log('igual');
        job.find('.job-desc').slideUp();
        job.toggleClass('active');
      }else{
        job.find('.job-desc').slideDown();
        $(active).toggleClass('active');
        $(active).find('.job-desc').slideUp();
        job.toggleClass('active');
      }
    }else{
      // console.log('nao tem active');
      job.find('.job-desc').slideDown();
      job.toggleClass('active');
    }
    var btn = job.find('.apply-button');
    if(!isElementInViewport(btn)){
      $('html, body').animate({
            scrollTop: job.offset().top-180
          }, 1000);
    }
  });
	Loader.stopLoader();
...
},{"./loader.js":4,"./populate-jobs.js":5,"./translate.js":7}],2:[function(require,module,exports){
module.exports={
...
module.exports=[
  {
    "jobTitle": "Desenvolvedor Back-End Java EE",
    "jobDesc": "Buscamos profissional para atuar na arquitetura, desenvolvimento e integração de software, para tornar o produto escalável, acompanhando o plano de expansão a nível nacional. <br /><b>Requisitos para Desenvolvedor Back-End Java EE:</b> Conhecimento de Enterprise Java Beans - EJB 3.1, Servidor de aplicação JBoss, WebServices / JSON. É desejável conhecimento de linguagem SQL e banco de dados MySQL.",
    "mailSubject": "Vaga Desenvolvedor Back-End Java EE"
  },
  {
    "jobTitle": "Desenvolvedor de Software Mobile",
    "jobDesc": "Buscamos desenvolvedores de software que nos ajudarão a criar e evoluir nossas aplicações mobile. <br /><b>Requisitos para Desenvolvedor Mobile:</b>Experiência com desenvolvimento de aplicações para Windows Phone, Android e iOS. Não é necessário ser um 'guru' de desenvolvimento mobile, mas ter confiança sobre sua capacidade de aprender e desenvolver softwares para uma destas plataformas.",
    "mailSubject": "Vaga Desenvolvedor de Software Mobile"
  },
  {
    "jobTitle": "Desenvolvedor de Software Web",
    "jobDesc": "Buscamos desenvolvedores de software que nos ajudarão a criar e evoluir nossas aplicações web. <br /><b>Requisitos para Desenvolvedor Web:</b>Experiência com desenvolvimento de front-end usando HTML5, CSS3, JavaScript, jQuery, Bootstrap. Conhecimento em AngularJS ou ReactJS é desejável.",
    "mailSubject": "Vaga Desenvolvedor Web"
  },
  {
    "jobTitle": "Desenvolvedor de Software Back-End",
    "jobDesc": "<b>Requisitos para Desenvolvedor Back-End:</b> Experiência com desenvolvimento back-end, acessando banco de dados relacional, e servindo webServices JSON sobre HTTP. Sólida experiência com SQL. Usamos node.js no nosso servidor, mas não é necessário domínio desta linguagem, ainda que seja um diferencial.",
    "mailSubject": "Vaga Desenvolvedor de SW Back-End"
  },
  {
    "jobTitle": "Arquiteto/Analista de Business Intelligence",
    "jobDesc": "Buscamos profissional para garantir a integridade das informações para suportar as ações de marketing das empresas, por meio da realização de carga, tratamento e extração de dados no Banco de Dados seguindo critérios pré-definidos, para tomada de decisões, além de gerar relatórios analíticos para gerenciamento efetivo de campanhas. Fará especificação de Requisitos, estruturação de Data Lakes. Também construirá extração, transformação e carregamento (ETL) usando o conjunto de ferramentas de integração de dados em modelos de dados star-schema e snowflake, criação de cubos multimensionais. Será responsável pelo desenvolvimento de Relatórios Interativos, Gráficos, Dashboards, painéis e mapas, a partir de ferramentas especialistas, para plataforma web e mobile, fornecendo visões estratégicas de negócio.<br /><b>Requisitos para Arquiteto/Analista de Business Intelligence:</b> Experiência em ferramentas como Qlik ou Tableau. Sólidos conhecimentos de SQL. Desejáveis conhecimentos de MongoDB, Hadoop e Phyton. Capacidade de comunicação/apresentação, de forma gráfica e oral, os resultados de suas análises. Graduação em Ciências da Computação, Engenharia da Computação, ou áreas afins.",
    "mailSubject": "Vaga Arquiteto/Analista de Business Intelligence"
  },
  {
    "jobTitle": "Cientista de Dados",
    "jobDesc": "Buscamos profissional para realizar a análise e visualização de dados, criação de rotinas de preparação de dados para análise, análise quantitativa de dados, aplicação de modelos estatísticos a grandes volumes de dados. Será responsável pela criação de relatórios e representações gráficas dos resultados de análises. <br /><b>Requisitos para Cientista de Dados:</b> Experiência com ferramentas de análise e visualização de dados. Experiência na criação de rotinas de preparação de dados para análise. Vivência na aplicação de modelos estatísticos a grandes volumes de dados. Experiência na criação de relatórios e representações gráficas dos resultados de análises. Sólidos conhecimentos de Estatística, SQL. Conhecimento de MongoDB, Hadoop, Phyton, Java e linguagem R. Capacidade de comunicação/apresentação, de forma gráfica e oral, os resultados de suas análises. Graduação em Matemática, Física, Estatística, com desejável formação complementar em TI, ou graduação em Ciências da Computação, Engenharia da Computação, ou áreas afins.",
    "mailSubject": "Vaga Cientista de Dados"
  }
]
...
var loadJobs = function(){
  ...
  for(var i =0; i< jobs.length; i++ ){
    var job = jobs[i];
    var element = $("<li class='job job-"+i+"'>"  +
                     "<h1 class='job-title'><i class='fa'></i>"+job.jobTitle+"</h1>"+
                     "<div class='job-desc'>"+job.jobDesc+
                        "<a href='mailto:querofazerparte@4all.com?subject="+encodeURI(job.mailSubject)+"'>"+
                          "<div class='apply-button'>"+
                            "querofazerparte@4all.com"+
                          "</div>"+
                        "</a>"+
                      "</div>"+
                    "</li>");
...
},{"./jobs.json":3}],6:[function(require,module,exports){
module.exports={
